document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderTasks = () => {
        ...
        if (filteredTasks.length === 0) {
            taskListContainer.innerHTML = `<p class="text-gray-500 text-center col-span-full">Tidak ada tugas...</p>`;
        } else {
    ...
    const handleFormSubmit = (e) => {
        ...
        if (new Date(deadline) < new Date().setHours(0, 0, 0, 0)) {
            showError("Tanggal deadline tidak valid!");
            return;
        }

        // Logika menambah atau mengedit tugas
